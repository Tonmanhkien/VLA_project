...
from .encoder import ImageEncoder, TextEncoder, StateEncoder
...
class VLADiffusionPolicy(nn.Module):
    def __init__(self, vocab_size, state_dim, action_dim,
        ...
        super().__init__()
        self.img_encoder = ImageEncoder(d_model=d_model)
        self.txt_encoder = TextEncoder(vocab_size=vocab_size, d_word=64, d_model=d_model)
        self.state_encoder = StateEncoder(state_dim=state_dim, d_model=d_model)
